...
interface PointCloudOctree {
  pointBudget: number;
  pcoGeometry: PointCloudGreyhoundGeometry | PointCloudEptGeometry;
  boundingBox: THREE.Box3;
  boundingSphere: THREE.Sphere;
  material: PointCloudMaterial;
  visiblePointsTarget: number;
  minimumNodPixelSize: number;
  level: number;
  position: any;
...
class PointCloudOctree extends PointCloudTree {
  constructor(
    ...
    //TODO: pcogeometry is a different geo.
    //@ts-ignore
  ...
  toTreeNode(geometryNode, parent) {
    ...
    sceneNode.onBeforeRender = (
      ...
      // TODO: material.program does not exist
      if (material.program) {
        ...
        // TODO: visibleNodeTextureOffsets is not a property
        // so this will always be false and not run
  ...
  getPointsInProfile(profile, maxDepth, callback) {
    ...
    for (let i = 0; i < profile.points.length - 1; i++) {
      ...
      // TODO: ps does not return anything
   ...
   *
   */
  pick(viewer, camera, ray, params: any = {}) {
        ...
        if (-0.5 < pos.x && pos.x < 0.5) {
          if (-0.5 < pos.y && pos.y < 0.5) {
            if (-0.5 < pos.z && pos.z < 0.5) {
              // vector4 cannot be directly applied to vector1
              // @ts-ignore
    ...
    for (let node of this.visibleNodes) {
      ...
      for (let i = 0; i < buffer.numElements; i++) {
        ...
        if (-0.5 < pos.x && pos.x < 0.5) {
          if (-0.5 < pos.y && pos.y < 0.5) {
            if (-0.5 < pos.z && pos.z < 0.5) {
              // vector 4 cannot be directly applied to vector 3
              //@ts-ignore
              shrinkedLocalBounds.expandByPoint(pos);
            }
          }
