...
import { PointSizeType, PointColorType, ClipTask } from './Presets';
...
class Renderer {
  ...
  createBuffer(geometry) {
    ...
    //TODO: createVertextArray doesn't Exist
    //@ts-ignore
    webglBuffer.vao = gl.createVertexArray();
    webglBuffer.numElements = geometry.attributes.position.count;

    //TODO: bindVertextArray doesn't exist
    //@ts-ignore
    ...
    //TODO: bindVertexArray doesn't exit?
    //@ts-ignore
  ...
  updateBuffer(geometry) {
    ...
    //TODO: bindVertexArray doesn't exist?
    //@ts-ignore
    gl.bindVertexArray(webglBuffer.vao);
  ...
  renderNodes(
    ...
    for (let node of nodes) {
...

      {
        ...
        shader.setUniform2f("uFilterGPSTimeClipRange", [
          gpsCliPRangeMin,
          gpsCliPRangeMax
        ]);
      }
  ...
  renderOctree(octree, nodes, camera, target, params: renderOctreeParams = {}) {
    let gl = this.gl;
