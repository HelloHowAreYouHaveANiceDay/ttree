...
class GreyhoundLoader {
   ...
   * finished
   */
  static load(url: string, callback) {
  ...
  loadPointAttributes(mno) {
    ...
    let pa = new PointAttributes(null);
  ...
  createChildAABB(aabb, childIndex) {
    let min = aabb.min;
    let max = aabb.max;
    let dHalfLength = new THREE.Vector3()
      .copy(max)
      .sub(min)
      .multiplyScalar(0.5);
...
export default GreyhoundLoader;