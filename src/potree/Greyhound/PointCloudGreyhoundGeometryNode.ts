...
interface PointCloudGreyhoundGeometryNode$1 {
  name: string;
  pcoGeometry: any;
  index: number;
  geometry: any;
...
class PointCloudGreyhoundGeometryNode$1 extends PointCloudTreeNode {
  constructor(name, pcoGeometry, boundingBox, scale, offset) {
    ...
    //@ts-ignore
    this.id = PointCloudGreyhoundGeometryNode$1.IDCount++;
    this.name = name;
  ...
  loadHierarchyThenPoints() {
    ...
    let callback = function(node, greyhoundHierarchy) {
      ...
      for (let i = 0; i < decoded.length; i++) {
        ...
        //TODO: check why createChildAABB is not considered a part of the type
        //@ts-ignore
        let boundingBox = GreyhoundLoader.createChildAABB(
          parentNode.boundingBox,
          index
        );

        let currentNode = new PointCloudGreyhoundGeometryNode$1(
          name,
          pgg,
          boundingBox,
          node.scale,
...
export default PointCloudGreyhoundGeometryNode$1;
