...
interface VolumePanel {
  viewer: Viewer;
  elContent: JQuery;
  elDownloadButton: JQuery;
  elCopyRotation: JQuery;
  elCheckClip: JQuery;
  elCheckShow: JQuery;
  elCopyScale?: JQuery;
  elRemove?: JQuery;
...
class VolumePanel extends MeasurePanel {
  constructor(viewer, measurement, propertiesPanel) {
    ...
    let copyIconPath = "/static/resources" + "/icons/copy.svg";
  ...
  async download() {
    ...
    let handle = null;
    {
      ...
      let url = `${
        this.viewer.server
      }/create_regions_filter?pointclouds=[${pointcloudsArg}]&regions=[${regionsArg}]`;
...

    {
      ...
      let handleFinish = jsResponse => {
        ...
        for (let i = 0; i < jsResponse.pointclouds.length; i++) {
          let url = `${
            this.viewer.server
          }/download_regions_filter_result?handle=${handle}&index=${i}`;
        ...
        let reportURL = `${
          this.viewer.server
        }/download_regions_filter_report?handle=${handle}`;
      ...
      let handleUnexpected = jsResponse => {
        let message = `Unexpected Response. <br>status: ${
          jsResponse.status
        } <br>message: ${jsResponse.message}`;
        info(message);
      };
