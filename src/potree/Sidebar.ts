...
import { CameraMode, ClipTask, ClipMethod } from "./Presets";
...
import ZoomableSlider from "./ZoomableSlider";
...
class Sidebar {
  ...
  initToolbar() {
    ...
    elToolbar.append(
      this.createToolIcon(
        "/static/icons/sphere_distances.svg",
        "[title]tt.volume_measurement",
        () => {
  ...
  initScene() {
...

    {
      ...
      let geoJSONIcon = `/static/icons/file_geojson.svg`;
      let dxfIcon = `/static/icons/file_dxf.svg`;
    ...
    for (let volume of [
      ...
      ...this.viewer.scene.polygonClipVolumes
    ]) {
  ...
  initGPSTimeFilters() {
    ...
    this.viewer.addEventListener(
      "filter_gps_time_range_changed",
      onGPSTimeChanged
    );
    this.viewer.addEventListener(
      "filter_gps_time_extent_changed",
      onGPSTimeExtentChanged
    );
  }
  ...
  initAccordion() {
    ...
    let languages = [["EN", "en"], ["FR", "fr"], ["DE", "de"], ["JP", "jp"]];
