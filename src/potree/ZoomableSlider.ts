interface ZoomableSlider {
  visibleRange: [number, number];
  chosenRange: [number, number];
  step: number;
...
class ZoomableSlider {
  constructor() {
    ...
    this.elLabelVisibleLeft = this.element.querySelector(
      "[name=label_visible_left]"
    );
    this.elLabelVisibleRight = this.element.querySelector(
      "[name=label_visible_right]"
    );

    this.elLabelChosenLeft = this.element.querySelector(
      "[name=label_chosen_left]"
    );
    this.elLabelChosenRight = this.element.querySelector(
    ...
    let onMouseDown = e => {
      ...
      let value =
        e.target === this.elLeft ? this.chosenRange[0] : this.chosenRange[1];
    ...
    let onMouseMove = e => {
      ...
      let valueDelta =
        (this.visibleRange[1] - this.visibleRange[0]) * normalizedDelta;
  ...
  update() {
    ...
    let normalizedLeft =
      (chosenRange[0] - visibleRange[0]) / (visibleRange[1] - visibleRange[0]);
    let normalizedRight =
      (chosenRange[1] - visibleRange[0]) / (visibleRange[1] - visibleRange[0]);
    let pixelLeft =
      Math.round(normalizedLeft * pixelWidth) - elLeft.clientWidth / 2;
    let pixelRight =
      Math.round(normalizedRight * pixelWidth) - elRight.clientWidth / 2;
    ...
    let visibleLeftString = addThousandsSeparatorsToString(
      visibleRange[0].toFixed(precision)
    );
    this.elLabelVisibleLeft.innerHTML = `${visibleLeftString}`;

    this.elLabelVisibleRight.style.right = "0px";
    let visibleRightString = addThousandsSeparatorsToString(
      visibleRange[1].toFixed(precision)
    );

    this.elLabelVisibleRight.innerHTML = `${visibleRightString}`;

    this.elLabelChosenLeft.style.left = `${pixelLeft}px`;
    let chosenLeftString = addThousandsSeparatorsToString(
      chosenRange[0].toFixed(precision)
    );
    this.elLabelChosenLeft.innerHTML = `${chosenLeftString}`;

    this.elLabelChosenRight.style.left = `${pixelRight}px`;
    let chosenRightString = addThousandsSeparatorsToString(
      chosenRange[1].toFixed(precision)
    );
