...
import { ClipTask, ClipMethod, CameraMode } from "./Presets";
...
interface Viewer {
  ...
  filterGPSTimeExtent: [number, number];
  //renderers
  potreeRenderer: any;
  edlRenderer: any;
  renderer: any;
...
class Viewer extends EventDispatcher {
        ...
        i18n.init(
          {
            lng: "en",
            //@ts-ignore
            resGetPath: "static/resouces/lang/__lng__/__ns__.json",
            preload: ["en", "fr", "de", "jp"],
            getAsync: true,
            debug: false
          },
  ...
  updateAnnotations() {
    ...
    this.scene.annotations.traverse(annotation => {
      ...
      let radius = annotation.boundingBox.getBoundingSphere(new THREE.Sphere())
        .radius;
